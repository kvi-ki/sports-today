...
var [tsMajor, tsMinor] = ts6.versionMajorMinor
  .split('.')
...
function isAccessExpression(node) {
  return (
    ts7.isPropertyAccessExpression(node) || ts7.isElementAccessExpression(node)
  );
}
...
function isAccessorDeclaration(node) {
  return (
    ts7.isGetAccessorDeclaration(node) || ts7.isSetAccessorDeclaration(node)
  );
}
...
function isAssignmentPattern(node) {
  return (
    ts7.isObjectLiteralExpression(node) || ts7.isArrayLiteralExpression(node)
  );
}
...
function isBindingOrAssignmentElementTarget(node) {
  return (
    ...
    ts7.isIdentifier(node) ||
    ts7.isPropertyAccessExpression(node) ||
    ts7.isElementAccessExpression(node) ||
    ts7.isOmittedExpression(node)
  );
}
...
function isBlockLike(node) {
  return (
    ts7.isSourceFile(node) ||
    ts7.isBlock(node) ||
    ts7.isModuleBlock(node) ||
    ts7.isCaseOrDefaultClause(node)
  );
}
...
function isDeclarationName(node) {
  return (
    ts7.isIdentifier(node) ||
    ts7.isPrivateIdentifier(node) ||
    ts7.isStringLiteralLike(node) ||
    ts7.isNumericLiteral(node) ||
    ts7.isComputedPropertyName(node) ||
    ts7.isElementAccessExpression(node) ||
...
function isDeclarationWithTypeParameterChildren(node) {
  return (
    ...
    ts7.isInterfaceDeclaration(node) ||
    ts7.isTypeAliasDeclaration(node) ||
    ts7.isJSDocTemplateTag(node)
  );
}
function isDeclarationWithTypeParameters(node) {
  return (
    isDeclarationWithTypeParameterChildren(node) ||
    ts7.isJSDocTypedefTag(node) ||
    ts7.isJSDocCallbackTag(node) ||
    ts7.isJSDocSignature(node)
  );
}
function isDestructuringPattern(node) {
  return (
    isBindingPattern(node) ||
    ts7.isObjectLiteralExpression(node) ||
    ts7.isArrayLiteralExpression(node)
  );
}
...
function isFunctionLikeDeclaration(node) {
  return (
    ts7.isFunctionDeclaration(node) ||
    ts7.isMethodDeclaration(node) ||
    ts7.isGetAccessorDeclaration(node) ||
    ts7.isSetAccessorDeclaration(node) ||
    ts7.isConstructorDeclaration(node) ||
    ts7.isFunctionExpression(node) ||
    ts7.isArrowFunction(node)
  );
}
function hasDecorators(node) {
  return (
    ts7.isParameter(node) ||
    ts7.isPropertyDeclaration(node) ||
    ts7.isMethodDeclaration(node) ||
    ts7.isGetAccessorDeclaration(node) ||
    ts7.isSetAccessorDeclaration(node) ||
    ts7.isClassExpression(node) ||
    ts7.isClassDeclaration(node)
  );
}
function hasExpressionInitializer(node) {
  return (
    ts7.isVariableDeclaration(node) ||
    ts7.isParameter(node) ||
    ts7.isBindingElement(node) ||
    ts7.isPropertyDeclaration(node) ||
    ts7.isPropertyAssignment(node) ||
    ts7.isEnumMember(node)
  );
}
function hasInitializer(node) {
  return (
    hasExpressionInitializer(node) ||
    ts7.isForStatement(node) ||
    ts7.isForInStatement(node) ||
    ts7.isForOfStatement(node) ||
    ts7.isJsxAttribute(node)
  );
}
function hasJSDoc(node) {
  if (
    ...
    ts7.isArrowFunction(node) ||
    ts7.isBlock(node) ||
    ts7.isBreakStatement(node) ||
    ts7.isCallSignatureDeclaration(node) ||
    ts7.isCaseClause(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    ts7.isConstructorDeclaration(node) ||
    ts7.isConstructorTypeNode(node) ||
    ts7.isConstructSignatureDeclaration(node) ||
    ts7.isContinueStatement(node) ||
    ts7.isDebuggerStatement(node) ||
    ts7.isDoStatement(node) ||
    ts7.isEmptyStatement(node) ||
    isEndOfFileToken(node) ||
    ts7.isEnumDeclaration(node) ||
    ts7.isEnumMember(node) ||
    ts7.isExportAssignment(node) ||
    ts7.isExportDeclaration(node) ||
    ts7.isExportSpecifier(node) ||
    ts7.isExpressionStatement(node) ||
    ts7.isForInStatement(node) ||
    ts7.isForOfStatement(node) ||
    ts7.isForStatement(node) ||
    ts7.isFunctionDeclaration(node) ||
    ts7.isFunctionExpression(node) ||
    ts7.isFunctionTypeNode(node) ||
    ts7.isIfStatement(node) ||
    ts7.isImportDeclaration(node) ||
    ts7.isImportEqualsDeclaration(node) ||
    ts7.isIndexSignatureDeclaration(node) ||
    ts7.isInterfaceDeclaration(node) ||
    ts7.isJSDocFunctionType(node) ||
    ts7.isLabeledStatement(node) ||
    ts7.isMethodDeclaration(node) ||
    ts7.isMethodSignature(node) ||
    ts7.isModuleDeclaration(node) ||
    ts7.isNamedTupleMember(node) ||
    ts7.isNamespaceExportDeclaration(node) ||
    ts7.isParameter(node) ||
    ts7.isParenthesizedExpression(node) ||
    ts7.isPropertyAssignment(node) ||
    ts7.isPropertyDeclaration(node) ||
    ts7.isPropertySignature(node) ||
    ts7.isReturnStatement(node) ||
    ts7.isShorthandPropertyAssignment(node) ||
    ts7.isSpreadAssignment(node) ||
    ts7.isSwitchStatement(node) ||
    ts7.isThrowStatement(node) ||
    ts7.isTryStatement(node) ||
    ts7.isTypeAliasDeclaration(node) ||
    ts7.isVariableDeclaration(node) ||
    ts7.isVariableStatement(node) ||
    ts7.isWhileStatement(node) ||
    ts7.isWithStatement(node)
  ) {
    return true;
  }
  ...
  if (
    ...
    (ts7.isBinaryExpression(node) ||
      ts7.isElementAccessExpression(node) ||
      ts7.isIdentifier(node) ||
      ts7.isJSDocSignature(node) ||
      ts7.isObjectLiteralExpression(node) ||
      ts7.isPropertyAccessExpression(node) ||
      ts7.isTypeParameterDeclaration(node))
  ) {
    return true;
  }
  return false;
...
function hasType(node) {
  return (
    ...
    ts7.isPropertySignature(node) ||
    ts7.isPropertyDeclaration(node) ||
    ts7.isTypePredicateNode(node) ||
    ts7.isParenthesizedTypeNode(node) ||
    ts7.isTypeOperatorNode(node) ||
    ts7.isMappedTypeNode(node) ||
    ts7.isAssertionExpression(node) ||
    ts7.isTypeAliasDeclaration(node) ||
    ts7.isJSDocTypeExpression(node) ||
    ts7.isJSDocNonNullableType(node) ||
    ts7.isJSDocNullableType(node) ||
    ts7.isJSDocOptionalType(node) ||
    ts7.isJSDocVariadicType(node)
  );
}
function hasTypeArguments(node) {
  return (
    ts7.isCallExpression(node) ||
    ts7.isNewExpression(node) ||
    ts7.isTaggedTemplateExpression(node) ||
    ts7.isJsxOpeningElement(node) ||
    ts7.isJsxSelfClosingElement(node)
  );
}
function isJSDocComment(node) {
  ...
  if (isTsVersionAtLeast(4, 4)) {
    return (
      ts7.isJSDocLink(node) ||
      ts7.isJSDocLinkCode(node) ||
      ts7.isJSDocLinkPlain(node)
    );
  }
  return false;
}
...
function isJsxAttributeValue(node) {
  return (
    ts7.isStringLiteral(node) ||
    ts7.isJsxExpression(node) ||
    ts7.isJsxElement(node) ||
    ts7.isJsxSelfClosingElement(node) ||
    ts7.isJsxFragment(node)
  );
}
...
function isLiteralToken(node) {
  return (
    ts7.isNumericLiteral(node) ||
    ts7.isBigIntLiteral(node) ||
    ts7.isStringLiteral(node) ||
    ts7.isJsxText(node) ||
    ts7.isRegularExpressionLiteral(node) ||
    ts7.isNoSubstitutionTemplateLiteral(node)
  );
}
...
function isObjectBindingOrAssignmentPattern(node) {
  return (
    ts7.isObjectBindingPattern(node) || ts7.isObjectLiteralExpression(node)
  );
}
...
function isPseudoLiteralToken(node) {
  return (
    ts7.isTemplateHead(node) ||
    ts7.isTemplateMiddle(node) ||
    ts7.isTemplateTail(node)
  );
}
function isSignatureDeclaration(node) {
  return (
    ...
    ts7.isFunctionDeclaration(node) ||
    ts7.isMethodDeclaration(node) ||
    ts7.isConstructorDeclaration(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isAccessorDeclaration(node) ||
    ts7.isFunctionExpression(node) ||
    ts7.isArrowFunction(node)
  );
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  if (
    ts7.isImportClause(node) ||
    ts7.isImportEqualsDeclaration(node) ||
    ts7.isNamespaceImport(node) ||
    ts7.isImportOrExportSpecifier(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(5, 0) &&
    (ts7.isExportDeclaration(node) || ts7.isNamespaceExport(node))
  ) {
    return true;
  }
  return false;
...
function isTypeReferenceType(node) {
  return (
    ts7.isTypeReferenceNode(node) || ts7.isExpressionWithTypeArguments(node)
  );
}
...
function isVariableLikeDeclaration(node) {
  return (
    ...
    ts7.isShorthandPropertyAssignment(node) ||
    ts7.isEnumMember(node) ||
    ts7.isJSDocPropertyTag(node) ||
    ts7.isJSDocParameterTag(node)
  );
}
...
function isConstAssertionExpression(node) {
  return (
    ts8.isTypeReferenceNode(node.type) &&
    ts8.isIdentifier(node.type.typeName) &&
...
function isJSDocNamespaceDeclaration(node) {
  return (
    ts8.isModuleDeclaration(node) &&
    ts8.isIdentifier(node.name) &&
    (node.body === void 0 || isJSDocNamespaceBody(node.body))
  );
}
function isJsxTagNamePropertyAccess(node) {
  return (
    ts8.isPropertyAccessExpression(node) && // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts < 5
...
function isPropertyAccessEntityNameExpression(node) {
  return (
    ts8.isPropertyAccessExpression(node) &&
    ts8.isIdentifier(node.name) &&
...
function isSuperElementAccessExpression(node) {
  return (
    ts8.isElementAccessExpression(node) && isSuperExpression(node.expression)
  );
}
function isSuperPropertyAccessExpression(node) {
  return (
    ts8.isPropertyAccessExpression(node) && isSuperExpression(node.expression)
  );
}
...
function isAssignmentKind(kind) {
  return (
    kind >= ts10.SyntaxKind.FirstAssignment &&
    kind <= ts10.SyntaxKind.LastAssignment
  );
}
...
function isValidPropertyAccess(
  text,
  languageVersion = ts10.ScriptTarget.Latest
) {
  if (text.length === 0) return false;
  let ch = text.codePointAt(0);
  if (!ts10.isIdentifierStart(ch, languageVersion)) return false;
  for (let i = charSize(ch); i < text.length; i += charSize(ch)) {
    ch = text.codePointAt(i);
    if (!ts10.isIdentifierPart(ch, languageVersion)) return false;
  }
  return true;
}
...
var IntrinsicTypeFlags =
  ts11.TypeFlags.Intrinsic ??
  ts11.TypeFlags.Any |
    ts11.TypeFlags.Unknown |
    ts11.TypeFlags.String |
    ts11.TypeFlags.Number |
    ts11.TypeFlags.BigInt |
    ts11.TypeFlags.Boolean |
    ts11.TypeFlags.BooleanLiteral |
    ts11.TypeFlags.ESSymbol |
    ts11.TypeFlags.Void |
    ts11.TypeFlags.Undefined |
    ts11.TypeFlags.Null |
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) && isObjectFlagSet(type, ts13.ObjectFlags.EvolvingArray)
  );
}
...
function isTypeReference(type) {
  return (
    isObjectType(type) && isObjectFlagSet(type, ts13.ObjectFlags.Reference)
  );
}
...
function getWellKnownSymbolPropertyOfType(
  ...
  for (const prop of type.getProperties()) {
    ...
    if (
      ...
      !ts15.isComputedPropertyName(declaration.name)
    ) {
      continue;
    }
...
function isBindableObjectDefinePropertyCall(node) {
  return (
    ...
    ts16.isPropertyAccessExpression(node.expression) &&
    node.expression.name.escapedText === 'defineProperty' &&
    ts16.isIdentifier(node.expression.expression) &&
  ...
  while (true) {
    ...
    outer: switch (parent.kind) {
      ...
      case ts16.SyntaxKind.PrefixUnaryExpression:
        if (current.kind !== ts16.SyntaxKind.NumericLiteral) return false;
    ...
    if (getPropertyOfType(subType, name) === void 0) {
      const index =
        (isNumericPropertyName(name)
          ? typeChecker.getIndexInfoOfType(subType, ts17.IndexKind.Number)
          : void 0) ??
        typeChecker.getIndexInfoOfType(subType, ts17.IndexKind.String);
      if (index?.isReadonly) {
...
function isReadonlyAssignmentDeclaration(node, typeChecker) {
  ...
  const writableType =
    ...
    ts17.isPropertyAssignment(writableProp.valueDeclaration)
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
  return !!(
    (symbol.flags & ts17.SymbolFlags.Accessor) ===
      ts17.SymbolFlags.GetAccessor ||
    symbol.declarations?.some(
      (node) =>
        isModifierFlagSet(node, ts17.ModifierFlags.Readonly) ||
        (ts17.isVariableDeclaration(node) &&
          isNodeFlagSet(node.parent, ts17.NodeFlags.Const)) ||
        (ts17.isCallExpression(node) &&
          isReadonlyAssignmentDeclaration(node, typeChecker)) ||
        ts17.isEnumMember(node) ||
        ((ts17.isPropertyAssignment(node) ||
          ts17.isShorthandPropertyAssignment(node)) &&
