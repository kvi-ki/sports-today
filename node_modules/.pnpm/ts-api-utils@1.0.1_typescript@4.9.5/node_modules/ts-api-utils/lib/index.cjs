...
function isEqualsGreaterThanToken(node) {
  return (
    node.kind === import_typescript5.default.SyntaxKind.EqualsGreaterThanToken
  );
}
...
function isJsonMinusNumericLiteral(node) {
  return (
    node.kind === import_typescript5.default.SyntaxKind.PrefixUnaryExpression
  );
}
...
function isUnparsedSyntheticReference(node) {
  return (
    node.kind ===
    import_typescript5.default.SyntaxKind.UnparsedSyntheticReference
  );
}
...
var [tsMajor, tsMinor] = import_typescript6.default.versionMajorMinor
  .split('.')
...
function isAccessExpression(node) {
  return (
    import_typescript7.default.isPropertyAccessExpression(node) ||
    import_typescript7.default.isElementAccessExpression(node)
  );
}
...
function isAccessorDeclaration(node) {
  return (
    import_typescript7.default.isGetAccessorDeclaration(node) ||
    import_typescript7.default.isSetAccessorDeclaration(node)
  );
}
function isArrayBindingElement(node) {
  return (
    import_typescript7.default.isBindingElement(node) ||
    import_typescript7.default.isOmittedExpression(node)
  );
}
function isArrayBindingOrAssignmentPattern(node) {
  return (
    import_typescript7.default.isArrayBindingPattern(node) ||
    import_typescript7.default.isArrayLiteralExpression(node)
  );
}
function isAssignmentPattern(node) {
  return (
    import_typescript7.default.isObjectLiteralExpression(node) ||
    import_typescript7.default.isArrayLiteralExpression(node)
  );
}
function isBindingOrAssignmentElementRestIndicator(node) {
  if (
    import_typescript7.default.isSpreadElement(node) ||
    import_typescript7.default.isSpreadAssignment(node)
  ) {
    return true;
  }
...
function isBindingOrAssignmentElementTarget(node) {
  return (
    ...
    import_typescript7.default.isIdentifier(node) ||
    import_typescript7.default.isPropertyAccessExpression(node) ||
    import_typescript7.default.isElementAccessExpression(node) ||
    import_typescript7.default.isOmittedExpression(node)
  );
...
function isBindingPattern(node) {
  return (
    import_typescript7.default.isObjectBindingPattern(node) ||
    import_typescript7.default.isArrayBindingPattern(node)
  );
}
function isBlockLike(node) {
  return (
    import_typescript7.default.isSourceFile(node) ||
    import_typescript7.default.isBlock(node) ||
    import_typescript7.default.isModuleBlock(node) ||
    import_typescript7.default.isCaseOrDefaultClause(node)
  );
}
...
function isClassLikeDeclaration(node) {
  return (
    import_typescript7.default.isClassDeclaration(node) ||
    import_typescript7.default.isClassExpression(node)
  );
}
...
function isDeclarationName(node) {
  return (
    import_typescript7.default.isIdentifier(node) ||
    import_typescript7.default.isPrivateIdentifier(node) ||
    import_typescript7.default.isStringLiteralLike(node) ||
    import_typescript7.default.isNumericLiteral(node) ||
    import_typescript7.default.isComputedPropertyName(node) ||
    import_typescript7.default.isElementAccessExpression(node) ||
...
function isDeclarationWithTypeParameterChildren(node) {
  return (
    ...
    import_typescript7.default.isInterfaceDeclaration(node) ||
    import_typescript7.default.isTypeAliasDeclaration(node) ||
    import_typescript7.default.isJSDocTemplateTag(node)
  );
}
function isDeclarationWithTypeParameters(node) {
  return (
    isDeclarationWithTypeParameterChildren(node) ||
    import_typescript7.default.isJSDocTypedefTag(node) ||
    import_typescript7.default.isJSDocCallbackTag(node) ||
    import_typescript7.default.isJSDocSignature(node)
  );
}
...
function isEntityNameOrEntityNameExpression(node) {
  return (
    import_typescript7.default.isEntityName(node) ||
    isEntityNameExpression(node)
  );
}
function isForInOrOfStatement(node) {
  return (
    import_typescript7.default.isForInStatement(node) ||
    import_typescript7.default.isForOfStatement(node)
  );
}
function isFunctionLikeDeclaration(node) {
  return (
    import_typescript7.default.isFunctionDeclaration(node) ||
    import_typescript7.default.isMethodDeclaration(node) ||
    import_typescript7.default.isGetAccessorDeclaration(node) ||
    import_typescript7.default.isSetAccessorDeclaration(node) ||
    import_typescript7.default.isConstructorDeclaration(node) ||
    import_typescript7.default.isFunctionExpression(node) ||
    import_typescript7.default.isArrowFunction(node)
  );
}
function hasDecorators(node) {
  return (
    import_typescript7.default.isParameter(node) ||
    import_typescript7.default.isPropertyDeclaration(node) ||
    import_typescript7.default.isMethodDeclaration(node) ||
    import_typescript7.default.isGetAccessorDeclaration(node) ||
    import_typescript7.default.isSetAccessorDeclaration(node) ||
    import_typescript7.default.isClassExpression(node) ||
...
function hasExpressionInitializer(node) {
  return (
    import_typescript7.default.isVariableDeclaration(node) ||
    import_typescript7.default.isParameter(node) ||
    import_typescript7.default.isBindingElement(node) ||
    import_typescript7.default.isPropertyDeclaration(node) ||
    import_typescript7.default.isPropertyAssignment(node) ||
    import_typescript7.default.isEnumMember(node)
  );
}
function hasInitializer(node) {
  return (
    hasExpressionInitializer(node) ||
    import_typescript7.default.isForStatement(node) ||
    import_typescript7.default.isForInStatement(node) ||
    import_typescript7.default.isForOfStatement(node) ||
    import_typescript7.default.isJsxAttribute(node)
  );
}
function hasJSDoc(node) {
  if (
    ...
    import_typescript7.default.isArrowFunction(node) ||
    import_typescript7.default.isBlock(node) ||
    import_typescript7.default.isBreakStatement(node) ||
    import_typescript7.default.isCallSignatureDeclaration(node) ||
    import_typescript7.default.isCaseClause(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isClassLikeDeclaration(node) ||
    import_typescript7.default.isConstructorDeclaration(node) ||
    import_typescript7.default.isConstructorTypeNode(node) ||
    import_typescript7.default.isConstructSignatureDeclaration(node) ||
    import_typescript7.default.isContinueStatement(node) ||
    import_typescript7.default.isDebuggerStatement(node) ||
    import_typescript7.default.isDoStatement(node) ||
    import_typescript7.default.isEmptyStatement(node) ||
    isEndOfFileToken(node) ||
    import_typescript7.default.isEnumDeclaration(node) ||
    import_typescript7.default.isEnumMember(node) ||
    import_typescript7.default.isExportAssignment(node) ||
    import_typescript7.default.isExportDeclaration(node) ||
    import_typescript7.default.isExportSpecifier(node) ||
    import_typescript7.default.isExpressionStatement(node) ||
    import_typescript7.default.isForInStatement(node) ||
    import_typescript7.default.isForOfStatement(node) ||
    import_typescript7.default.isForStatement(node) ||
    import_typescript7.default.isFunctionDeclaration(node) ||
    import_typescript7.default.isFunctionExpression(node) ||
    import_typescript7.default.isFunctionTypeNode(node) ||
    import_typescript7.default.isIfStatement(node) ||
    import_typescript7.default.isImportDeclaration(node) ||
    import_typescript7.default.isImportEqualsDeclaration(node) ||
    import_typescript7.default.isIndexSignatureDeclaration(node) ||
    import_typescript7.default.isInterfaceDeclaration(node) ||
    import_typescript7.default.isJSDocFunctionType(node) ||
    import_typescript7.default.isLabeledStatement(node) ||
    import_typescript7.default.isMethodDeclaration(node) ||
    import_typescript7.default.isMethodSignature(node) ||
    import_typescript7.default.isModuleDeclaration(node) ||
    import_typescript7.default.isNamedTupleMember(node) ||
    import_typescript7.default.isNamespaceExportDeclaration(node) ||
    import_typescript7.default.isParameter(node) ||
    import_typescript7.default.isParenthesizedExpression(node) ||
    import_typescript7.default.isPropertyAssignment(node) ||
    import_typescript7.default.isPropertyDeclaration(node) ||
    import_typescript7.default.isPropertySignature(node) ||
    import_typescript7.default.isReturnStatement(node) ||
    import_typescript7.default.isShorthandPropertyAssignment(node) ||
    import_typescript7.default.isSpreadAssignment(node) ||
    import_typescript7.default.isSwitchStatement(node) ||
    import_typescript7.default.isThrowStatement(node) ||
    import_typescript7.default.isTryStatement(node) ||
    import_typescript7.default.isTypeAliasDeclaration(node) ||
    import_typescript7.default.isVariableDeclaration(node) ||
    import_typescript7.default.isVariableStatement(node) ||
    import_typescript7.default.isWhileStatement(node) ||
    import_typescript7.default.isWithStatement(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(4, 4) &&
    import_typescript7.default.isClassStaticBlockDeclaration(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(5, 0) &&
    (import_typescript7.default.isBinaryExpression(node) ||
      import_typescript7.default.isElementAccessExpression(node) ||
      import_typescript7.default.isIdentifier(node) ||
      import_typescript7.default.isJSDocSignature(node) ||
      import_typescript7.default.isObjectLiteralExpression(node) ||
      import_typescript7.default.isPropertyAccessExpression(node) ||
      import_typescript7.default.isTypeParameterDeclaration(node))
  ) {
    return true;
  }
  return false;
...
function hasType(node) {
  return (
    ...
    import_typescript7.default.isPropertySignature(node) ||
    import_typescript7.default.isPropertyDeclaration(node) ||
    import_typescript7.default.isTypePredicateNode(node) ||
    import_typescript7.default.isParenthesizedTypeNode(node) ||
    import_typescript7.default.isTypeOperatorNode(node) ||
    import_typescript7.default.isMappedTypeNode(node) ||
    import_typescript7.default.isAssertionExpression(node) ||
    import_typescript7.default.isTypeAliasDeclaration(node) ||
    import_typescript7.default.isJSDocTypeExpression(node) ||
    import_typescript7.default.isJSDocNonNullableType(node) ||
    import_typescript7.default.isJSDocNullableType(node) ||
    import_typescript7.default.isJSDocOptionalType(node) ||
    import_typescript7.default.isJSDocVariadicType(node)
  );
}
function hasTypeArguments(node) {
  return (
    import_typescript7.default.isCallExpression(node) ||
    import_typescript7.default.isNewExpression(node) ||
    import_typescript7.default.isTaggedTemplateExpression(node) ||
    import_typescript7.default.isJsxOpeningElement(node) ||
    import_typescript7.default.isJsxSelfClosingElement(node)
  );
}
function isJSDocComment(node) {
  ...
  if (isTsVersionAtLeast(4, 4)) {
    return (
      import_typescript7.default.isJSDocLink(node) ||
      import_typescript7.default.isJSDocLinkCode(node) ||
      import_typescript7.default.isJSDocLinkPlain(node)
    );
  }
  return false;
}
...
function isJsonObjectExpression(node) {
  return (
    ...
    import_typescript7.default.isNumericLiteral(node) ||
    import_typescript7.default.isStringLiteral(node) ||
...
function isJsxAttributeLike(node) {
  return (
    import_typescript7.default.isJsxAttribute(node) ||
    import_typescript7.default.isJsxSpreadAttribute(node)
  );
}
function isJsxAttributeValue(node) {
  return (
    import_typescript7.default.isStringLiteral(node) ||
    import_typescript7.default.isJsxExpression(node) ||
    import_typescript7.default.isJsxElement(node) ||
    import_typescript7.default.isJsxSelfClosingElement(node) ||
    import_typescript7.default.isJsxFragment(node)
  );
}
function isJsxChild(node) {
  return (
    import_typescript7.default.isJsxText(node) ||
    import_typescript7.default.isJsxExpression(node) ||
    import_typescript7.default.isJsxElement(node) ||
    import_typescript7.default.isJsxSelfClosingElement(node) ||
    import_typescript7.default.isJsxFragment(node)
...
function isLiteralToken(node) {
  return (
    import_typescript7.default.isNumericLiteral(node) ||
    import_typescript7.default.isBigIntLiteral(node) ||
    import_typescript7.default.isStringLiteral(node) ||
    import_typescript7.default.isJsxText(node) ||
    import_typescript7.default.isRegularExpressionLiteral(node) ||
    import_typescript7.default.isNoSubstitutionTemplateLiteral(node)
  );
}
...
function isModuleReference(node) {
  return (
    import_typescript7.default.isEntityName(node) ||
    import_typescript7.default.isExternalModuleReference(node)
  );
}
function isNamedImportBindings(node) {
  return (
    import_typescript7.default.isNamespaceImport(node) ||
    import_typescript7.default.isNamedImports(node)
  );
}
function isNamedImportsOrExports(node) {
  return (
    import_typescript7.default.isNamedImports(node) ||
    import_typescript7.default.isNamedExports(node)
  );
}
...
function isObjectTypeDeclaration(node) {
  return (
    ...
    isClassLikeDeclaration(node) ||
    import_typescript7.default.isInterfaceDeclaration(node) ||
    import_typescript7.default.isTypeLiteralNode(node)
  );
}
...
function isPseudoLiteralToken(node) {
  return (
    import_typescript7.default.isTemplateHead(node) ||
    import_typescript7.default.isTemplateMiddle(node) ||
    import_typescript7.default.isTemplateTail(node)
  );
}
function isSignatureDeclaration(node) {
  return (
    ...
    import_typescript7.default.isFunctionDeclaration(node) ||
    import_typescript7.default.isMethodDeclaration(node) ||
    import_typescript7.default.isConstructorDeclaration(node) || // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts <5
    isAccessorDeclaration(node) ||
    import_typescript7.default.isFunctionExpression(node) ||
    import_typescript7.default.isArrowFunction(node)
  );
...
function isTypeOnlyCompatibleAliasDeclaration(node) {
  if (
    import_typescript7.default.isImportClause(node) ||
    import_typescript7.default.isImportEqualsDeclaration(node) ||
    import_typescript7.default.isNamespaceImport(node) ||
    import_typescript7.default.isImportOrExportSpecifier(node)
  ) {
    return true;
  }
  if (
    isTsVersionAtLeast(5, 0) &&
    (import_typescript7.default.isExportDeclaration(node) ||
      import_typescript7.default.isNamespaceExport(node))
  ) {
    return true;
  }
  return false;
...
function isTypeReferenceType(node) {
  return (
    import_typescript7.default.isTypeReferenceNode(node) ||
    import_typescript7.default.isExpressionWithTypeArguments(node)
  );
}
function isUnionOrIntersectionTypeNode(node) {
  return (
    import_typescript7.default.isUnionTypeNode(node) ||
    import_typescript7.default.isIntersectionTypeNode(node)
  );
}
function isUnparsedSourceText(node) {
  return (
    import_typescript7.default.isUnparsedPrepend(node) ||
    import_typescript7.default.isUnparsedTextLike(node)
  );
}
function isVariableLikeDeclaration(node) {
  return (
    ...
    import_typescript7.default.isShorthandPropertyAssignment(node) ||
    import_typescript7.default.isEnumMember(node) ||
    import_typescript7.default.isJSDocPropertyTag(node) ||
    import_typescript7.default.isJSDocParameterTag(node)
  );
}
...
function isConstAssertionExpression(node) {
  return (
    import_typescript8.default.isTypeReferenceNode(node.type) &&
    import_typescript8.default.isIdentifier(node.type.typeName) &&
...
function isJSDocNamespaceDeclaration(node) {
  return (
    import_typescript8.default.isModuleDeclaration(node) &&
    import_typescript8.default.isIdentifier(node.name) &&
    (node.body === void 0 || isJSDocNamespaceBody(node.body))
  );
}
function isJsxTagNamePropertyAccess(node) {
  return (
    import_typescript8.default.isPropertyAccessExpression(node) && // eslint-disable-next-line deprecation/deprecation -- Keep compatibility with ts < 5
...
function isPropertyAccessEntityNameExpression(node) {
  return (
    import_typescript8.default.isPropertyAccessExpression(node) &&
    import_typescript8.default.isIdentifier(node.name) &&
...
function isSuperElementAccessExpression(node) {
  return (
    import_typescript8.default.isElementAccessExpression(node) &&
...
function isAssignmentKind(kind) {
  return (
    kind >= import_typescript10.default.SyntaxKind.FirstAssignment &&
    kind <= import_typescript10.default.SyntaxKind.LastAssignment
  );
}
...
function isValidPropertyAccess(
  text,
  languageVersion = import_typescript10.default.ScriptTarget.Latest
) {
...
var IntrinsicTypeFlags =
  import_typescript11.default.TypeFlags.Intrinsic ??
  import_typescript11.default.TypeFlags.Any |
    import_typescript11.default.TypeFlags.Unknown |
    import_typescript11.default.TypeFlags.String |
    import_typescript11.default.TypeFlags.Number |
    import_typescript11.default.TypeFlags.BigInt |
    import_typescript11.default.TypeFlags.Boolean |
    import_typescript11.default.TypeFlags.BooleanLiteral |
    import_typescript11.default.TypeFlags.ESSymbol |
    import_typescript11.default.TypeFlags.Void |
    import_typescript11.default.TypeFlags.Undefined |
    import_typescript11.default.TypeFlags.Null |
    import_typescript11.default.TypeFlags.Never |
    import_typescript11.default.TypeFlags.NonPrimitive;
...
function isIndexedAccessType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.IndexedAccess
  );
}
function isInstantiableType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.Instantiable
  );
}
function isIntersectionType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.Intersection
  );
}
...
function isStringMappingType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.StringMapping
  );
}
function isSubstitutionType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.Substitution
  );
}
function isTypeParameter(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.TypeParameter
  );
}
function isTypeVariable(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.TypeVariable
  );
}
...
function isUnionOrIntersectionType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.UnionOrIntersection
  );
}
function isUniqueESSymbolType(type) {
  return isTypeFlagSet(
    type,
    import_typescript12.default.TypeFlags.UniqueESSymbol
  );
}
...
function isEvolvingArrayType(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, import_typescript13.default.ObjectFlags.EvolvingArray)
  );
}
function isTupleType(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, import_typescript13.default.ObjectFlags.Tuple)
  );
}
function isTypeReference(type) {
  return (
    isObjectType(type) &&
    isObjectFlagSet(type, import_typescript13.default.ObjectFlags.Reference)
  );
}
...
function isBooleanLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript14.default.TypeFlags.BooleanLiteral
  );
}
function isBigIntLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript14.default.TypeFlags.BigIntLiteral
  );
}
...
function isNumberLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript14.default.TypeFlags.NumberLiteral
  );
}
function isStringLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript14.default.TypeFlags.StringLiteral
  );
}
function isTemplateLiteralType(type) {
  return isTypeFlagSet(
    type,
    import_typescript14.default.TypeFlags.TemplateLiteral
  );
}
...
function getWellKnownSymbolPropertyOfType(
  ...
  for (const prop of type.getProperties()) {
    ...
    if (
      ...
      !import_typescript15.default.isComputedPropertyName(declaration.name)
    ) {
      continue;
    }
...
function isBindableObjectDefinePropertyCall(node) {
  return (
    ...
    import_typescript16.default.isPropertyAccessExpression(node.expression) &&
    node.expression.name.escapedText === 'defineProperty' &&
    import_typescript16.default.isIdentifier(node.expression.expression) &&
  ...
  while (true) {
    ...
    outer: switch (parent.kind) {
      ...
      case import_typescript16.default.SyntaxKind.PrefixUnaryExpression:
        if (
          current.kind !== import_typescript16.default.SyntaxKind.NumericLiteral
        )
          return false;
...
function isFalsyType(type) {
  if (
    isTypeFlagSet(
      type,
      import_typescript17.default.TypeFlags.Undefined |
        import_typescript17.default.TypeFlags.Null |
        import_typescript17.default.TypeFlags.Void
    )
  )
    return true;
  ...
  return typeParts.some((subType) => {
    ...
    return !!(
      ...
      (
        isSymbolFlagSet(
          prop,
          import_typescript17.default.SymbolFlags.ValueModule
...
function isReadonlyPropertyFromMappedType(type, name, typeChecker) {
  if (
    !isObjectType(type) ||
    !isObjectFlagSet(type, import_typescript17.default.ObjectFlags.Mapped)
  )
    return;
  ...
  if (declaration.readonlyToken !== void 0 && !/^__@[^@]+$/.test(name))
    return (
      ...
      import_typescript17.default.SyntaxKind.MinusToken
    );
      ...
      const index =
        (isNumericPropertyName(name)
          ? typeChecker.getIndexInfoOfType(
              subType,
              import_typescript17.default.IndexKind.Number
            )
          : void 0) ??
        typeChecker.getIndexInfoOfType(
          subType,
          import_typescript17.default.IndexKind.String
        );
      if (index?.isReadonly) {
...
function isReadonlyAssignmentDeclaration(node, typeChecker) {
  ...
  const writableType =
    ...
    import_typescript17.default.isPropertyAssignment(
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
  return !!(
    (symbol.flags & import_typescript17.default.SymbolFlags.Accessor) ===
      import_typescript17.default.SymbolFlags.GetAccessor ||
    symbol.declarations?.some(
      (node) =>
        isModifierFlagSet(
          node,
          import_typescript17.default.ModifierFlags.Readonly
        ) ||
        (import_typescript17.default.isVariableDeclaration(node) &&
          isNodeFlagSet(
            node.parent,
            import_typescript17.default.NodeFlags.Const
          )) ||
        (import_typescript17.default.isCallExpression(node) &&
          isReadonlyAssignmentDeclaration(node, typeChecker)) ||
        import_typescript17.default.isEnumMember(node) ||
        ((import_typescript17.default.isPropertyAssignment(node) ||
          import_typescript17.default.isShorthandPropertyAssignment(node)) &&
